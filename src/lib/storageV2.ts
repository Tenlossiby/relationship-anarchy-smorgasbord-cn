...
import { CONTENT_VERSION, createEnvelope, createProfile as createDomainProfile, decodeEnvelope, describePerspective, digestString, encodeEnvelope, isAnswerEffective, makeId, migrateV1Profile, normalizeAnswer, validateProfile } from '@/lib/domain';
...
function legacyStatuses(answer: CardAnswerV2): StatusLabel[] { const statuses: StatusLabel[] = []; if (answer.stance === 'want') statuses.push('agree'); if (answer.stance === 'unsure') statuses.push('maybe'); if (answer.stance === 'not_for_me') statuses.push('absolutely_not'); if (answer.stance === 'hard_limit') statuses.push('hard_limit'); for (const marker of answer.markers || []) statuses.push(marker === 'important' ? 'necessary' : marker); if (answer.participation === 'self') statuses.push('will_do'); if (answer.participation === 'other') statuses.push('they_will'); if (answer.participation === 'together') statuses.push('together'); return statuses; }
...
export const exportProfileToText = (profile: ProfileV2, includesNotes = true): string => { const envelope = createEnvelope(profile, includesNotes); return ['关系安那其自助拼盘 - 关系档案', '', `档案：${profile.title}`, `填写视角：${describePerspective(profile.direction.author.displayName, profile.direction.subject.displayName)}`, `回答数：${countEffectiveAnswers(profile)} · 包含备注：${includesNotes ? '是' : '否'}`, '隐私提示：Base64 只是编码，不是加密；内容仅在本地处理。', '', '---RAS_DATA_START---', encodeEnvelope(envelope), '---RAS_DATA_END---', ''].join('\n'); };
...
export const generateShareCode = (profile: ProfileV2): string => exportProfileToText(profile, true);
