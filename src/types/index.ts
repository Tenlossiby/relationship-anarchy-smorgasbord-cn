...
export const STATUS_LABELS: Record<StatusLabel, { en: string; zh: string; color: string }> = {
  agree: { en: 'Agree', zh: '同意', color: '#7A9B76' }, necessary: { en: 'Necessary', zh: '必要', color: '#5B8DBE' }, maybe: { en: 'Maybe', zh: '也许', color: '#D4A84B' }, future_possible: { en: 'Future Possible', zh: '未来可能', color: '#A0A05B' }, need_discussion: { en: 'Need Discussion', zh: '需要讨论', color: '#5BA0A0' }, absolutely_not: { en: 'Absolutely Not', zh: '绝对不行', color: '#D46B6B' }, hard_limit: { en: 'Hard Limit', zh: '硬性界限', color: '#C75B5B' }, will_do: { en: 'I will', zh: '我会（旧）', color: '#7A9B76' }, they_will: { en: 'They will', zh: '对方会（旧）', color: '#5B8DBE' }, together: { en: 'Together', zh: '共同参与（旧）', color: '#9B7AA0' }, unwilling: { en: 'Unwilling', zh: '不愿意（旧）', color: '#D4845B' },
};

export interface PersonRef { id: string; displayName: string; }
export interface CardAnswerV2 { cardId: string; statuses?: StatusLabel[]; participation?: Participation; note?: string; updatedAt: string; stance?: Stance; markers?: AnswerMarker[]; legacy?: { rawStatuses: string[]; needsReview: boolean }; }
